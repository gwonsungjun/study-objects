...
public class movieTest {
    ...
    private final DiscountPolicy avatarDiscountPolicy = new AmountDiscountPolicy(
            given_고정할인금액,
    ...
    Movie avatar = new Movie("아바타", Duration.ofMinutes(120), Money.wons(10000), avatarDiscountPolicy);

    private final DiscountPolicy titanicDiscountPolicy = new PercentDiscountPolicy(
            given_할인비율,
            new SequenceCondition(2),
            new PeriodCondition(DayOfWeek.TUESDAY, LocalTime.of(14, 0), LocalTime.of(16, 59)),
            new PeriodCondition(DayOfWeek.THURSDAY, LocalTime.of(10, 0), LocalTime.of(13, 59)));

    Movie titanic = new Movie("타이타닉", Duration.ofMinutes(200), Money.wons(10000), titanicDiscountPolicy);

    private final DiscountPolicy starwarsDiscountPolicy = new NoneDiscountPolicy();

    Movie starwars = new Movie("스타워즈", Duration.ofMinutes(210), Money.wons(10000), starwarsDiscountPolicy);
    ...
    class Describe_calculateMovieFee {
            ...
            class Context_with_valid_period {
                ...
                void it_returns_discounted_fee() {
                    for (Screening 할인되는_시간에_시작하는_상영 : givenScreens()) {
                        ...
                        Assertions.assertEquals(basicFee().minus(given_고정할인금액), 계산된_요금);
                    }
                }
            }
            ...
            class Context_with_invalid_period {
                ...
                void it_returns_fee_does_not_discounted() {
                    for (Screening 할인되는_시간에_시작하는_상영 : givenScreens()) {
                        ...
                        Assertions.assertEquals(basicFee(), 계산된_요금);
                    }
                }
            }
        ...
        class Context_with_titanic extends TestCalculateMovieFee {
            ...
            class Context_with_valid_period {
                ...
                void it_returns_discounted_fee() {
                    for (Screening 할인되는_시간에_시작하는_상영 : givenScreens()) {
                        ...
                        Assertions.assertEquals(basicFee().times(1 - given_할인비율), 계산된_요금);
                    }
                }
            }
            ...
            @DisplayName("상영 순번이 할인 조건에 맞지 않는다면")
            class Context_with_invalid_seq {
                final List<Integer> 지정되지_않은_상영_순번 = List.of(1, 3, 5, 6, 7, 8, 9);
        ...
        class Context_with_starwars extends TestCalculateMovieFee {
            ...
            @Test
            @DisplayName("할인 되지 않은 금액을 리턴한다.")
            void it_returns_fee_does_not_discounted() {
                final Screening 상영 = new Screening(givenMovie(), 1, given_일요일_오후);
                final Money 계산된_요금 = subject(상영);

                Assertions.assertEquals(basicFee(), 계산된_요금, "할인되지 않았으므로 금액은 변함이 없다");
            }
        }
    }
    ...
    class Describe_changeDiscountPolicy {
        ...
        class Context_with_starwars {
            ...
            class Context_with_avatar_discount_policy {
                final DiscountPolicy given_할인정책 = avatarDiscountPolicy;
            ...
            class Context_with_titanic_discount_policy {
                final DiscountPolicy given_할인정책 = titanicDiscountPolicy;
                ...
                void it_changes_the_discount_policy() {
...

                    {
                        ...
                        final Screening 상영 = new Screening(스타워즈, 타이타닉_할인_조건_순번, given_일요일_오후);
                        final Money 계산된_요금 = 스타워즈.calculateMovieFee(상영);
